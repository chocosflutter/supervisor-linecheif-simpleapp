...
- **ALERTS cross-device sync (Phase 10) — DONE:** alerts/notifications used to be client-memory-only (raiseAlert/resolveAlert never touched Supabase). Now: alert ids are real UUIDs (crypto.randomUUID), `raiseAlert` → `enqueueTable("alerts","insert",…)`, `resolveAlert` → `enqueueTable("alerts","update",…,{id})`, and `hydrateFromSupabase` loads alerts (RLS-scoped to accessible lines, latest 200). Migration `phase10_alerts_entry_ref_and_names` added `entry_ref` (deep-link key like `prod-<id>`) + denormalized `raised_by_name`/`resolved_by_name` so alerts round-trip without joins. `updateProductionHour` is dead code (edits go through addProductionHour upsert). **VERIFIED server-side (rolled-back txns):** supervisor `sync_attendance`/`sync_production`/`sync_downtime` all insert 1 row; IE `sync_load_style` creates active line_style + line_style_costs; `sync_production` → `line_day_agg` trigger → `get_line_kpis` returns the aggregate. Chief/IE KPI graphs are React-Query-backed (`useKpis`/`useKpisByGroup` → `get_line_kpis`); `setOnline(true)` runs flush → hydrate → `queryClient.invalidateQueries()`, so graphs auto-refresh with latest server data on reconnect (also `refetchOnReconnect`).
